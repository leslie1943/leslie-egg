...
class HomeController extends Controller {
  async index() {
    // ------------------ cookies ------------------
    this.ctx.cookies.set('name', 'suzhen')

    // ------------------ sessions ------------------
    this.ctx.session.visited = this.ctx.session.visited ? ++this.ctx.session.visited : 1
    ...
    const template = `<div style="width:vmin;height:100vh; padding-top:20px; background-color:rgb(217, 236, 255); text-align:center; font-size:14px;">
      ...
      <p>Hi, this name from cookie attribute: ${this.ctx.cookies.get('name')}</p>
      <p>Hi, this count from session attribute: you visited this page ${this.ctx.session.visited} times</p>
